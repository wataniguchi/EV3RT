...
class CheckPinkColor(Behaviour):
    ...
    def update(self) -> Status:
        ...
        if 100<r<120 and 60<g<75 and 75<b<93:
                return Status.SUCCESS
        else:
            return Status.RUNNING
...
class TraceLineColor(Behaviour):
    ...
    def update(self) -> Status:
        ...
        self.logger.debug("%s.get_brightness(): %s" % (self.__class__.__name__, g_color_sensor.get_brightness()))
        ...
        #以下お試し→無理？
        #RGB値を取得して青の成分を使う
        # rgb = g_color_sensor.get_raw_color()
        # blue_value = rgb[2]  # RGBのB成分（青色）

        # # PID制御に青の成分を入力
        # turn = int(self.pid(blue_value))
...
def build_behaviour_tree() -> BehaviourTree:
    ...
    carry_04.add_children(
        [
            RunAsInstructed(name="rotate", pwm_r=40,pwm_l=-40),
            IsRotated(name="check rotated", delta_dire=60),
        ]
    )
    ...
    carry_051.add_children(
        ...
        # スマートキャリーまで直進
    carry_052.add_children(
        [
            RunAsInstructed(name="go straight",pwm_l=50,pwm_r=50),
            IsDistanceEarned(name="check distance", delta_dist = 1415),
        ]
    )
