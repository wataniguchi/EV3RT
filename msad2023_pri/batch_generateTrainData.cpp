/*
  ...
  g++ batch_generateTrainData.cpp -std=c++17 `pkg-config --cflags --libs opencv4` -I.. -o batch_generateTrainData
*/
...
int main() 
{
  ...
  if ( imgArray.empty() ) {
    _log("*** ERROR - image files NOT found");
    return EXIT_FAILURE;
  }
  ...
  while (true) {
    ...
    if (pattern) {
      ...
      std::ostringstream ossPan, ossSmp;
      ossPan << TRAINING_PATH << "/pan" << std::setw(2) << std::setfill('0') << pattern << "-" << std::setw(13) << std::setfill('0') << ms << ".jpg";
      _log("writing panorama as file - %s", ossPan.str().c_str());
      imwrite(ossPan.str(), pano);
      ossSmp << TRAINING_PATH << "/smp" << std::setw(2) << std::setfill('0') << pattern << "-" << std::setw(13) << std::setfill('0') << ms << ".jpg";
      _log("writing panorama simple as file - %s", ossSmp.str().c_str());
      imwrite(ossSmp.str(), pano_simple);
      /* delete all the original image files */
      int ret = 0;
      for (std::string path : imgPathArray) {
	...
	_log("*** ERROR - image files NOT deleted");
	return EXIT_FAILURE;
      }
    }  
