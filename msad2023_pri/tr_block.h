...
#define TR_BLOCK1_L \
  /* BLOCK1: make a turn, down the arm, and try to find the Treasure Block at the initial position */ \
  .composite<BrainTree::MemSequence>() \
    /* section L101: set guide angle and location */ \
    .leaf<SetGuideAngle>() \
    .leaf<SetGuideLocation>() \
    /* section L102: face toward the area and arm down */ \
    .leaf<RotateEV3>(prof->getValueAsNum("BLOCK_L102_DEGREE"), \
		     prof->getValueAsNum("BLOCK_L102_SPEED"), 0.0) \
    .leaf<ArmUpDownFull>(AD_DOWN) \
    .leaf<IsFoundBlock>(prof->getValueAsNum("BLOCK_GS_MIN"), \
            prof->getValueAsNum("BLOCK_GS_MAX"), \
            prof->getValueAsNumVec("BLOCK_BGR_MIN_TRE"), \
...
#define TR_BLOCK6_L	\
  /* BLOCK6: move to the course line and to Goal */ \
  .composite<BrainTree::MemSequence>() \
    /* section L601: move toward the course line until getting out of the lattice area */ \
    .composite<BrainTree::ParallelSequence>(1,2) \
      .leaf<IsXdiffFromGuideLocationLarger>(prof->getValueAsNum("BLOCK_L601_XDIFF")) \
      .leaf<RunPerGuideAngle>(prof->getValueAsNum("BLOCK_L601_OFFSET"), \
	    prof->getValueAsNum("BLOCK_L601_SPEED"), \
	    prof->getValueAsNumVec("BLOCK_ANG_PID_CONST")) \
    .end() \
    .leaf<StopNow>() \
    /* section L602: move toward the course line */ \
    .composite<BrainTree::ParallelSequence>(1,2) \
      ...
      .leaf<RunPerGuideAngle>(prof->getValueAsNum("BLOCK_L602_OFFSET"), \
	    ...
	    prof->getValueAsNumVec("BLOCK_ANG_PID_CONST")) \
    .end() \
    /* section L603: move further until touching to the course line */ \
    .composite<BrainTree::ParallelSequence>(1,3) \
      .leaf<IsColorDetected>(CL_BLACK) \
      .leaf<IsDistanceEarned>(prof->getValueAsNum("BLOCK_L603_DIST")) \
      .leaf<RunPerGuideAngle>(prof->getValueAsNum("BLOCK_L603_OFFSET"), \
	    prof->getValueAsNum("BLOCK_L603_SPEED"), \
	    prof->getValueAsNumVec("BLOCK_ANG_PID_CONST")) \
    .end() \
