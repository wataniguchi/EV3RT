...
public:
  TraverseVLine(int s, int t, std::vector<double> pidSen, std::vector<double> pidCam, int gs_min, int gs_max,
    ...
    Status update() override {
	    ...
	    move = MV_ON_COLUMN;
	    decoyMoved = 0;
	    rowState[0] = rowState[1] = rowState[2] = rowState[3] = rowState[4] = RS_UNKNOWN;
	    detour = false;
            updated = true;
        }
	...
	case TVLST_ON_LINE:
	  if (isColor(CL_BLUE, cur_rgb)) {
	    ...
	    vLineRow = 3;
	    _log("ODO=%05d, *** WARNING - column number mismatch: X=%d, vLineColumnStartX=%d. forcefully set to Column 3", currentDist, plotter->getLocX(), vLineColumnStartX);
	    /* keep the state */
	  } else if (st != TVLST_INITIAL && move == MV_ON_COLUMN && vLineRow >= 3 && abs(vLineColumnStartX - plotter->getLocX()) >= 700 && directionOnColumn == -1) {
	    vLineRow = 2;
	    _log("ODO=%05d, *** WARNING - column number mismatch: X=%d, vLineColumnStartX=%d. forcefully set to Column 2", currentDist, plotter->getLocX(), vLineColumnStartX);
	    /* keep the state */
	  }
	  break;
	...
	case TVLST_ENTERING_CIRCLE:
	  if (move == MV_ON_COLUMN) {
	    if (rowState[vLineRow] == RS_NONE || (rowState[vLineRow] == RS_TREASURE && decoyMoved < 2)) { /* nothing to see */
	...
	if (st == TVLST_END) {
	  ...
	  leftMotor->setPWM(speed); /* magic number */
	  rightMotor->setPWM(speed); /* magic number */
	  return Status::Running;
