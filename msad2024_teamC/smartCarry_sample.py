...
class RotateDegrees(Behaviour):
    def __init__(self, name: str, power: int, target_angle: int):
        ...
        self.power = power
        self.target_angle = target_angle * g_course # 回転させたい角度（正の値で右回転、負の値で左回転）
        self.target_angle = target_angle * -1
...
def build_behaviour_tree() -> BehaviourTree:
    ...
    loop_01.add_children(
        [
            RunAsInstructed(name="run instructed", pwm_l=40, pwm_r=40),
            IsDistanceEarned(name="check distance", delta_dist = 120), #この部分は距離を測ってコードを変える。
        ]
    )
    # 回転部分
    loop_02.add_children(
        [
            RotateDegrees(name="routate",power=40,target_angle=90)
        ]
    )
    loop_03.add_children(
        [
            RunAsInstructed(name="run instructed", pwm_l=40, pwm_r=40),
            IsDistanceEarned(name="check distance", delta_dist = 300), #この部分は距離を測ってコードを変える。
        ]
    )
    # 回転部分
