...
# 2列目
    ...
    dbr_loop_18.add_children(
        [
            MoveStraight_dbr(name="back", power=-50, target_distance=75)
        ]
    )
    # 指定距離走行_after
...
# 3列目
    ...
    dbr_loop_34.add_children(
        [
            MoveStraight(name="move straight", power=40, target_distance=360),
        ]
    )
    # バック
    ...
    step_04B.add_children(
        [
            TraceLineCam(name="last run", power=40, pid_p=2.5, pid_i=0.0015, pid_d=0.1,
                         gs_min=0, gs_max=80, trace_side=TraceSide.CENTER),
            IsDistanceEarned(name="check distance 2", delta_dist = 850),
