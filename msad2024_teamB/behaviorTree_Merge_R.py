...
class IsColorDetected(Behaviour):
    ...
    def update(self) -> Status:
        ...
        #Black判定
        if self.name == "black" :
            if((5 < color[2] < 80) & (5 < color[1] < 80) & (5 < color[0] < 80)):
...
# 1列目
    ...
    dbr_loop_02.add_children(
        [
            MoveStraight_dbr(name="move straight", power=40, target_distance=210),
        ]
    )
    ...
    dbr_loop_05.add_children(
        [
            MoveStraightLR_dbr(name="move straight 4", right_power=-10, left_power=-60, target_distance=90),
        ]
    )
    dbr_loop_06.add_children(
        [
            MoveStraight_dbr(name="back", power=-50, target_distance=115)
        ]
    )
    # 指定距離走行_after
    dbr_loop_07.add_children(
        [
        TraceLineCam(name="trace normal edge", power=34, pid_p=1.0, pid_i=0.0015, pid_d=0.1,gs_min=0, gs_max=80, trace_side=TraceSide.OPPOSITE),
    ...
    dbr_loop_10.add_children(
        [
            MoveStraightLR(name="move straight 4", right_power=0, left_power=60, target_distance=205),
        ]
    )
    dbr_loop_11.add_children(
        ...
        TraceLineCam(name="trace normal edge", power=33, pid_p=0.8, pid_i=0.0015, pid_d=0.1,
                         gs_min=0, gs_max=80, trace_side=TraceSide.OPPOSITE),
        IsDistanceEarned_before(name="check distance", delta_dist = 900),
        IsRedColorDetected(name="check red color", threshold=12.0), 
        IsBlueColorDetected(name="check blue color", threshold=12.0), 
        ]
    )
    dbr_loop_12.add_children(
        [
            MoveStraight_dbr(name="move straight", power=40, target_distance=190),
        ]
    )
    ...
    dbr_loop_16.add_children(
        [
            MoveStraight_dbr(name="back", power=-50, target_distance=140)
        ]
    )
    # 指定距離走行_after
    ...
    step_02B.add_children(
        [
            MoveStraightLR(name="Turn 1", right_power=15, left_power=65, target_distance=280), #LEFT
            MoveStraight(name="free run 2", power=70, target_distance=1110),
    ...
    step_03B_3.add_children(
        [
            MoveStraightLR(name="Turn 3", right_power=50, left_power=0, target_distance=170),
    ...
    step_04B.add_children(
        [
            TraceLineCam(name="last run", power=45, pid_p=2.5, pid_i=0.0015, pid_d=0.1,
                         gs_min=0, gs_max=80, trace_side=TraceSide.CENTER),
    ...
    step_04B_3.add_children(
        [
            TraceLineCam(name="last run 2", power=40, pid_p=2.5, pid_i=0.0015, pid_d=0.1,
                         gs_min=0, gs_max=80, trace_side=TraceSide.CENTER),
            IsDistanceEarned(name="check distance 3", delta_dist = 300),
        ]
    )
