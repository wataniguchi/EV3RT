...
# 1列目
    ...
    loop_02.add_children(
        [
            MoveStraight_dbr(name="move straight", power=40, target_distance=135),
        ]
    )
    ...
    loop_06.add_children(
        [
            MoveStraight_dbr(name="back", power=-50, target_distance=80)
        ]
    )
    # 指定距離走行_after
...
# 4列目
    ...
    loop_43.add_children(
        ...
        TraceLineCam(name="trace normal edge", power=37, pid_p=0.8, pid_i=0.0015, pid_d=0.1,
                         gs_min=0, gs_max=80, trace_side=TraceSide.OPPOSITE),
        IsDistanceEarned(name="check distance", delta_dist = 1315),
        ]
    )
    # 右に90度回転
