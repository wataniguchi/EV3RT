...
def build_behaviour_tree() -> BehaviourTree:
    ...
    loop_07.add_children(
        [
        TraceLineCam(name="trace normal edge", power=40, pid_p=0.5, pid_i=0.0015, pid_d=0.1,gs_min=0, gs_max=80, trace_side=TraceSide.NORMAL),
        IsDistanceEarned_after(name="check distance"),
        ]
    )
    loop_08.add_children(
        [
            MoveStraight(name="move straight", power=40, target_distance=420),
        ]
    )
