...
# JUNCT_UPPER_THRESH = 50 'ベースコード（元）
# JUNCT_LOWER_THRESH = 30 'ベースコード（元）
...
def build_behaviour_tree() -> BehaviourTree:
    ...
    #        pid_p=1.5→2.0→1,7→1.6→1.4→1.5
            ...
            TraceLineCam(name="trace normal edge", power=70, pid_p=1.5, pid_i=0.0, pid_d=0.4,
                         gs_min=0, gs_max=80, trace_side=TraceSide.NORMAL),
            IsDistanceEarned(name="check distance", delta_dist = 2100),
        ]
    )

    #最初のストレート減速1段階目（実はカーブ1つ目 ms）
...
#            IsDistanceEarned(name="check distance", delta_dist = 1500),        
    ...
    loop_02.add_children(
        [
#            TraceLineCam(name="trace normal edge", power=40, pid_p=2.0, pid_i=0.001, pid_d=0.35,
