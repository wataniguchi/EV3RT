...
def build_behaviour_tree() -> BehaviourTree:
    ...
    loop_01_01 = Parallel(name="loop 01", policy=ParallelPolicy.SuccessOnOne())
    loop_01_02 = Parallel(name="loop 01", policy=ParallelPolicy.SuccessOnOne())
    ...
    loop_01_3_01 = Parallel(name="loop 01", policy=ParallelPolicy.SuccessOnOne())
    loop_01_3 = Parallel(name="loop 01", policy=ParallelPolicy.SuccessOnOne())
    loop_01_3_02 = Parallel(name="loop 01", policy=ParallelPolicy.SuccessOnOne())
    ...
    loop_01.add_children(
        [
#            TraceLineCam(name="trace normal edge", power=40, pid_p=1.5, pid_i=0.0015, pid_d=0.4, ok
    ...
    loop_01_01.add_children(
        [
#            TraceLineCam(name="trace normal edge", power=50, pid_p=1.5, pid_i=0.0015, pid_d=0.4, ok
    ...
    #最初のストレート最高速度
    loop_01_02.add_children(
        [
    #        TraceLineCam(name="trace normal edge", power=70, pid_p=1.5, pid_i=0.0015, pid_d=0.4,
    #        pid_p=1.5→2.0→1,7→1.6→1.4→1.5
    #        pid_p=1.5, pid_i=0.2, pid_d=0.05 ←　copilot曰く

    #        TraceLineCam(name="trace normal edge", power=60, pid_p=1.2, pid_i=0.0015, pid_d=0.4, ok

    #        TraceLineCam(name="trace normal edge", power=65, pid_p=0.5, pid_i=0.0015, pid_d=0.6,　たぶんng
    #        pid_p=1.5→0.5  pid_d=0.4→0.6
                                                
            TraceLineCam(name="trace normal edge", power=65, pid_p=1.4, pid_i=0.2, pid_d=0.05,
                         gs_min=0, gs_max=80, trace_side=TraceSide.NORMAL),
...
#            TraceLineCam(name="trace normal edge", power=45, pid_p=2.0, pid_i=0.0015, pid_d=0.3, ok
                         
            TraceLineCam(name="trace normal edge", power=45, pid_p=2.0, pid_i=0.0015, pid_d=0.3,
                         gs_min=0, gs_max=80, trace_side=TraceSide.NORMAL),
            IsDistanceEarned(name="check distance", delta_dist = 1700),

#            IsDistanceEarned(name="check distance", delta_dist = 2000),  ok          
        ]
    )

    #カーブ後の短い直線（加速1段階目_ms）
    loop_01_3_01.add_children(
        [                      
            TraceLineCam(name="trace normal edge", power=55, pid_p=1.5, pid_i=0.0015, pid_d=0.4,
                         gs_min=0, gs_max=80, trace_side=TraceSide.NORMAL),
...
#           TraceLineCam(name="trace normal edge", power=55, pid_p=1.5, pid_i=0.0015, pid_d=0.4, ok
                          
            TraceLineCam(name="trace normal edge", power=65, pid_p=1.5, pid_i=0.0015, pid_d=0.4,
                         gs_min=0, gs_max=80, trace_side=TraceSide.NORMAL),
            IsDistanceEarned(name="check distance", delta_dist = 1200),

#            IsDistanceEarned(name="check distance", delta_dist = 1500),  ok          
        ]
    )
    
    #カーブ後の短い直線（減速1段階目_ms）
    loop_01_3_02.add_children(
        [                      
            TraceLineCam(name="trace normal edge", power=55, pid_p=1.5, pid_i=0.0015, pid_d=0.4,
                         gs_min=0, gs_max=80, trace_side=TraceSide.NORMAL),
            IsDistanceEarned(name="check distance", delta_dist = 300),
...
#            power=45→30→20×動かない。ﾊﾟﾜｰ低いと動かない。
