...
def imu_test():
    ...
    while not lib.hub_imu_is_ready():
        ...
        time.sleep(0.1)
    print("IMU is ready.")
    # Wait for IMU to become stationary
    while not lib.hub_imu_is_stationary():
        print(".", end="")
        time.sleep(0.1)
    print("IMU is stationary.")
    for i in range(10): # 10 seconds
        if not lib.hub_imu_is_stationary():
            print("IMU is not stationary. Please keep it still.")
...
def imu_run_test():
    ...
    for i in range(SQUARE_SIDES):
        ...
        print(f'[heading] before turn = {heading_now}')
        heading += 90
