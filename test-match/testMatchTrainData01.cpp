/*
  ...
  matching algorithm: AKAZE to detect key points, brute-force to match key points

  how to use (from workspace directory):
  ./test-match/testMatchTrainData01
 
  how to compile:

  g++ testMatchTrainData01.cpp -std=c++17 `pkg-config --cflags --libs opencv4` -I.. -o testMatchTrainData01
*/
...
int main() 
{
  ...
  for (int i = 0; i < imgFilenameArray.size()-1; i++) {
    for (int j = i+1; j < imgFilenameArray.size(); j++) {
      Mat img1, img2, roi, desc1, desc2, img_match;
